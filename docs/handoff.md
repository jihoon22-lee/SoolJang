...
- 최종 갱신: **2026-08-06 (Task 25 PR1~PR4 로컬 검증 전부 완료 — v1.1.0 배포 후 사용자
  2차 UI/UX 피드백 5항목 전부 반영. PR1([#62](https://github.com/jihoon22-lee/SoolJang/pull/62))·
  PR2([#63](https://github.com/jihoon22-lee/SoolJang/pull/63))·PR3([#64](https://github.com/jihoon22-lee/SoolJang/pull/64))·
  PR4(`fix/stats-pivot-buttons`) 모두 GitHub Actions 대규모 장애로 CI/머지 대기 — 상세는
  `plan.md` §4 Task 25)**
...
- 현재 브랜치: `fix/stats-pivot-buttons`(Task 25 PR4, 마지막 PR). PR1~PR3 는 각각
  `fix/products-layout-filters`·`feat/category-manager-polish`·`feat/vendor-search` 에서
  별도로 CI/머지 대기 중. Task 1~17·20~24·23 전부 완료(Task 18 은 `adapter` 전략 + JSON
  모드로 확장, 외부 소스 7곳 중 1곳(데일리샷) 실등록).
...
- 버전: **`1.1.0`**([GitHub 릴리스](https://github.com/jihoon22-lee/SoolJang/releases/tag/v1.1.0))
...
| Task 25 PR1~PR4 전부(2026-08-06, 로컬 검증 전부 완료·GitHub Actions 장애로 CI/머지 대기) | 순수 프론트엔드(백엔드 변경 없음, 4개 PR 전부). **PR1**([#62](https://github.com/jihoon22-lee/SoolJang/pull/62) `fix/products-layout-filters`, 항목 1·2): `.product-table th` 가 `thead` 로 스코프되지 않아 이름 열이 나머지 열을 밀어내던 버그, `LabelOcrPanel` 마진이 형제 버튼 높이를 늘리던 버그, 레이아웃 폭, 구매일 필터, 모바일 필터 기본 접힘(`<details>` 로 구현하려다 `content-visibility:hidden` 때문에 실패해 `useState`+버튼으로 재구현, §5 새 함정). `npm run check` 440 passed. **PR2**([#63](https://github.com/jihoon22-lee/SoolJang/pull/63) `feat/category-manager-polish`, 항목 3): "주종 추가" 폼 토글화, 행 액션 버튼 900px 이상 축소, 트리를 제품 수 내림차순 정렬. `npm run check` 441 passed. **PR3**([#64](https://github.com/jihoon22-lee/SoolJang/pull/64) `feat/vendor-search`, 항목 4): `VendorsPage` 에 기존 `AutocompleteInput`+`rankByQuery`/`matchesQuery` 재사용으로 검색+자동완성. `npm run check` 441 passed(신규 3건). **PR4**(`fix/stats-pivot-buttons`, 항목 5): `PivotExplorer` 의 `<form className="field-row">` 가 필드 5개(홀수) + button-row 를 2열 grid 에 담아 `align-items:stretch` 때문에 "실행"/"CSV 내보내기" 버튼이 세로로 늘어나던 것을, `.field-row > .button-row { grid-column: 1 / -1 }` 한 줄로 고쳤다. `npm run check` 438 passed. 네 PR 모두 `vite build` 정상, 로그인된 실브라우저로 실데이터(제품 406종·구매처 64곳) 확인까지 마쳤다. **2026-08-06 15:22 UTC부터 GitHub Actions 자체의 대규모 장애가 발생해 네 PR 모두 CI 가 막혔다**(§5 새 함정 참조) — 사용자 승인을 받아 장애 해소를 기다리는 동안 PR을 전부 순서대로 만들어 뒀다. 근거는 `plan.md` Task 25 PR1~PR4 절 |
...
| **닫힌 `<details>` 의 내용은 `display` 를 덮어써도 강제로 보이게 할 수 없다(최신 Chrome)** | `.filter-panel:not([open]) > *:not(summary) { display: block }` 로 900px 이상에서 항상 펼쳐 보이게 하려 했는데, `getComputedStyle` 은 `display:block` 을 정상 보고하면서도 실제로는 그려지지 않았다(`getBoundingClientRect` 는 실제 크기를 보고하는데 화면엔 작은 빈 상자만 보임) | 최신 Chrome 은 닫힌 `<details>` 의 비-summary 자식에 `content-visibility:hidden` 을 적용한다(단순 `display:none` 규칙이 아니다) — `display` 오버라이드만으로는 이 은닉을 되돌릴 수 없다. "모바일 접힘 + 데스크톱 항상 펼침"처럼 열림 상태를 뷰포트별로 강제해야 하면 네이티브 `<details>` 대신 일반 `useState` + 버튼(`aria-expanded`) 조합으로 구현하고, CSS 는 그 상태 클래스만 토글한다(`ProductFilterPanel.tsx`, Task 25 PR1 에서 실제로 겪음, 2026-08-06) |
| **GitHub Actions 자체가 통째로 장애를 일으킬 수 있다 — CI 실패가 항상 코드 문제는 아니다** | PR 의 여러 잡이 전부 "Set up job" 단계에서 "Failed to resolve action download info: Service Unavailable" 로 실패, 재실행해도 반복 실패, 일부 잡은 큐에 걸린 채 진행이 안 됨. 웹훅 스로틀이 걸리면 push/PR 이벤트가 새 워크플로를 아예 트리거하지 않기도 한다 | `curl -s https://www.githubstatus.com/api/v2/status.json`(또는 `/incidents/unresolved.json`)로 GitHub 쪽 장애 여부를 가장 먼저 확인한다 — `indicator` 가 `"none"` 이 아니면(예: `"major"`) 코드를 의심하기 전에 장애가 먼저다. 실패 지점이 매 잡마다 "Set up job"(액션 다운로드/러너 배정)이고 이 PR 이 실제로 건드리지 않는 영역이면 장애일 가능성이 매우 높다. Task 25 PR1~PR4(2026-08-06)에서 실제로 겪음 — 15:22 UTC 시작, 몇 시간째 미해결, 웹훅 트리거까지 스로틀돼 나중에 올린 PR 들은 체크 자체가 안 뜨기도 함(`gh pr checks` 가 "no checks reported") |
