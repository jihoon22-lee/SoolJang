...
- 최종 갱신: **2026-08-06 (Task 25 진행 중 — `v1.1.0` 배포 후 사용자 2차 UI/UX 피드백 5항목
  대응. PR1(`fix/products-layout-filters`, PR [#62](https://github.com/jihoon22-lee/SoolJang/pull/62))
  로컬 검증 완료, GitHub Actions 대규모 장애로 CI/머지 대기 중. PR2(`feat/category-manager-polish`)
  로컬 검증 완료 — 상세는 `plan.md` §4 Task 25)**
...
- 현재 브랜치: `feat/category-manager-polish`(Task 25 PR2. PR1 은 `fix/products-layout-filters`
  에서 별도로 CI/머지 대기 중). Task 1~17·20~24·23 전부 완료(Task 18 은 `adapter` 전략 +
  JSON 모드로 확장, 외부 소스 7곳 중 1곳(데일리샷) 실등록).
...
| Task 25 PR1 — 내 술 레이아웃·필터(2026-08-06, PR [#62](https://github.com/jihoon22-lee/SoolJang/pull/62) `fix/products-layout-filters`, 로컬 검증 완료·CI/머지 대기) | 순수 프론트엔드(백엔드 변경 없음). 사용자가 v1.1.0 사용 중 스크린샷과 함께 보고한 5가지 문제 중 항목 1·2 를 실측으로 진단·수정했다: `.product-table th` 선택자가 `thead` 로 스코프되지 않아 tbody 행 헤더(제품명)에도 걸려 이름 열이 줄바꿈을 거부하고 나머지 열이 글자 단위로 쪼개지던 버그, `LabelOcrPanel` 트리거가 `.field` 마진 때문에 flex 줄 높이를 늘려 형제 버튼들이 함께 커지던 버그, 레이아웃 폭 조정, 구매일 필터·누락 정렬 옵션 4종 추가, 모바일 기본 접힘 필터 패널. `<details>` 로 구현하려다 실브라우저에서 `content-visibility:hidden` 때문에 실패하는 걸 발견해(§5 새 함정 참조) `useState`+버튼 조합으로 다시 구현했다. `npm --prefix web run check` 440 passed, `vite build` 정상. **로그인된 실브라우저(실데이터 406종)로 390/768/1280/1908px 전후 비교, 구매일 필터(406→1건), "/" 단축키 확인**. **PR을 올린 뒤 2026-08-06 15:22 UTC부터 GitHub Actions 자체의 대규모 장애("Partial/Major System Outage")가 발생해 CI가 막혔다** — 프론트엔드 검사(TypeScript quality)는 이미 통과했고 남은 실패는 전부 이 PR이 안 건드리는 백엔드/컨테이너 잡이며 실패 지점도 매번 GitHub 러너의 "Set up job"(액션 다운로드) 자체라 코드와 무관함을 확인했다. 사용자 승인을 받아 장애 해소를 기다리는 동안 PR2 로 넘어감. 근거는 `plan.md` Task 25 PR1 절, D149~D151 |
...
| **GitHub Actions 자체가 통째로 장애를 일으킬 수 있다 — CI 실패가 항상 코드 문제는 아니다** | PR 의 여러 잡이 전부 "Set up job" 단계에서 "Failed to resolve action download info: Service Unavailable" 로 실패, 재실행해도 반복 실패, 일부 잡은 큐에 걸린 채 진행이 안 됨 | `curl -s https://www.githubstatus.com/api/v2/status.json`(또는 `/incidents/unresolved.json`)로 GitHub 쪽 장애 여부를 가장 먼저 확인한다 — `indicator` 가 `"none"` 이 아니면(예: `"major"`) 코드를 의심하기 전에 장애가 먼저다. 실패 지점이 매 잡마다 "Set up job"(액션 다운로드/러너 배정)이고 이 PR 이 실제로 건드리지 않는 영역(예: 백엔드 미변경 PR 에서 Python 관련 잡만 실패)이면 장애일 가능성이 매우 높다. Task 25 PR1(2026-08-06)에서 실제로 겪음 — 15:22 UTC 시작, 몇 시간째(21:30 UTC 확인 시점까지) 미해결, 웹훅 트리거까지 스로틀돼 재시도도 소용없는 구간이 있었다 |
