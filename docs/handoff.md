...
- 최종 갱신: **2026-08-06 (Task 25 진행 중 — `v1.1.0` 배포 후 사용자 2차 UI/UX 피드백 5항목
  대응. PR1([#62](https://github.com/jihoon22-lee/SoolJang/pull/62))·PR2([#63](https://github.com/jihoon22-lee/SoolJang/pull/63))·
  PR3(`feat/vendor-search`) 로컬 검증 완료, GitHub Actions 대규모 장애로 CI/머지 대기 —
  상세는 `plan.md` §4 Task 25)**
...
- 현재 브랜치: `feat/vendor-search`(Task 25 PR3. PR1·PR2 는 각각 `fix/products-layout-filters`·
  `feat/category-manager-polish` 에서 별도로 CI/머지 대기 중). Task 1~17·20~24·23 전부
...
| Task 25 PR1~PR3(2026-08-06, 로컬 검증 완료·GitHub Actions 장애로 CI/머지 대기) | 순수 프론트엔드(백엔드 변경 없음). **PR1**([#62](https://github.com/jihoon22-lee/SoolJang/pull/62) `fix/products-layout-filters`, 항목 1·2): `.product-table th` 가 `thead` 로 스코프되지 않아 이름 열이 나머지 열을 밀어내던 버그, `LabelOcrPanel` 마진이 형제 버튼 높이를 늘리던 버그, 레이아웃 폭, 구매일 필터, 모바일 필터 기본 접힘. `<details>` 로 접힘을 구현하려다 실브라우저에서 `content-visibility:hidden` 때문에 실패해(§5 새 함정) `useState`+버튼으로 재구현. `npm run check` 440 passed. **PR2**([#63](https://github.com/jihoon22-lee/SoolJang/pull/63) `feat/category-manager-polish`, 항목 3): "주종 추가" 폼 토글화, 행 액션 버튼 900px 이상 축소, 트리를 제품 수 내림차순 정렬(모든 계층 재귀 적용). `npm run check` 441 passed. **PR3**(`feat/vendor-search`, 항목 4): `VendorsPage` 에 기존 `AutocompleteInput`+`rankByQuery`/`matchesQuery` 재사용(`ProductDetail.tsx` 와 같은 조합)으로 검색+자동완성 추가, 같은 쿼리로 목록도 실시간 필터링. `npm run check` 441 passed(신규 3건). 세 PR 모두 `vite build` 정상, 로그인된 실브라우저로 실데이터 확인까지 마쳤다. **2026-08-06 15:22 UTC부터 GitHub Actions 자체의 대규모 장애("Partial/Major System Outage")가 발생해 세 PR 모두 CI 가 막혔다** — 프론트엔드 검사는 이미 통과했거나(PR1의 TypeScript quality) 웹훅 스로틀로 아예 트리거되지 않았고(PR2/PR3), 실패 지점은 매번 GitHub 러너의 "Set up job"(액션 다운로드) 자체라 코드와 무관함을 확인했다(§5 새 함정 참조). 사용자 승인을 받아 장애 해소를 기다리는 동안 PR을 순서대로 먼저 만들어 뒀다. 근거는 `plan.md` Task 25 PR1~PR3 절, D149~D151(PR1)·D152~D154(PR2)·D149(PR3, 별도 브랜치라 번호가 겹친다 — 머지 시 정리) |
