...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-08-06 (**Task 25 진행 중** — v1.1.0 실사용 2차 피드백 5항목. PR1([#62](https://github.com/jihoon22-lee/SoolJang/pull/62))·PR2([#63](https://github.com/jihoon22-lee/SoolJang/pull/63))·PR3(`feat/vendor-search`)는 로컬 검증까지 완료, GitHub Actions 대규모 장애로 CI/머지 대기. PR4 진행 예정) |
...
| 다음 착수 Task | **Task 25 진행 중**(§4 참조) — PR1~PR3 머지 대기(GitHub Actions 장애), 남은 PR4(통계 피벗 버튼 높이)를 이어서 진행한다. 그 외 Task 는 전부 사용자가 원하는 시점에 결정할 선택 사항이다: **Q8**(GHCR pull 방식 전환, 급하지 않음), **Q9**(외부 소스 나머지 6곳 등록·Task 19 본 착수) |
| 현재 브랜치 | `feat/vendor-search`(PR1·PR2 는 각각 `fix/products-layout-filters`·`feat/category-manager-polish` 에서 별도로 머지 대기 중) |
| 진행 중 잔여 항목 | **Task 25 PR1~PR3(머지 대기)·PR4** 진행 중(§4 참조). 그 외엔 §6 Q8·Q9 뿐이며 둘 다 급하지 않은 선택 사항이다 |
...
| 25 | v1.1.0 실사용 2차 피드백(레이아웃·모바일·주종/구매처/통계 UX) | 🟡 PR1·PR2·PR3 로컬 검증 완료(GitHub Actions 장애로 머지 대기), PR4 진행 예정 | `fix/products-layout-filters`, `feat/category-manager-polish`, `feat/vendor-search`, `fix/stats-pivot-buttons` | [#62](https://github.com/jihoon22-lee/SoolJang/pull/62), [#63](https://github.com/jihoon22-lee/SoolJang/pull/63) |
...
항목 4 → `feat/vendor-search`, 항목 5 → `fix/stats-pivot-buttons`).

**PR1·PR2 는 로컬 검증까지 끝내고 각각 PR을 올렸으나**, 2026-08-06 15:22 UTC부터 시작된
**GitHub Actions 자체의 대규모 장애**(공식 상태 "Partial/Major System Outage" — 워크플로가
시작도 못 되거나 도중에 취소됨, 웹훅 트리거까지 스로틀됨)에 막혀 CI/머지 대기 중이다. 사용자
승인을 받아 장애 해소를 기다리는 동안 나머지 PR을 순서대로(`main` 기준 각각 별도 브랜치에서)
먼저 진행했다.
...
| # | 결정 | 근거 |
|---|---|---|
| D149 | `VendorsPage` 검색을 새 컴포넌트 없이 기존 `AutocompleteInput` + `search.ts::rankByQuery`/`matchesQuery` 조합으로 구현한다. 같은 `vendorQuery` state로 (a) 자동완성 드롭다운과 (b) 목록 자체의 실시간 필터링을 동시에 한다 | `ProductDetail.tsx` 의 구매처 이름 자동완성이 이미 같은 조합을 쓰고 있어(초성 검색 포함) 새로 설계·검증할 필요가 없었다. "자동완성"과 "스크롤이 긴 목록을 줄여 달라"는 요청 둘 다 이 한 조합으로 동시에 충족된다 — 별도 "점프 이동" 방식(자동완성 선택 시 스크롤만 이동)보다 목록 자체가 줄어드는 쪽이 사용자가 실제로 겪는 문제(스크롤 길이)를 직접 해결한다 |
