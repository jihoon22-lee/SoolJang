...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-08-20 (**Task 34 PR5 완료** — 프리셋 카탈로그(`infrastructure/external/presets.py`)에서 검증된 스펙으로 소스를 등록하고, `adapter_spec` v2(POST 검색+body, 정적 헤더, 자격 증명 헤더 주입, `strip_tags`)를 추가했다. robots.txt 를 실제 요청 호스트에서 받도록 고쳐(D187) 데일리샷의 다중 호스트 버그를 해결했다. 자격 증명은 Fernet 암호화로 저장. D187~D190. 이전 갱신은 PR4 완료 — 소스 헬스 체크(D185~D186)) |
...
| 다음 착수 Task | **Task 34 PR6 — 애매 구간 LLM 재판정**([실행 계획](plan-external-v2.md)). 월 호출 상한은 설정값(기본 200회)이라 착수를 막는 것이 없다 |
| 현재 브랜치 | `feat/external-source-presets`(Task 34 PR5, 머지 대기) |
| 진행 중 잔여 항목 | Task 34 PR6~PR7. 그 외엔 §9 백로그·§6 Q6 뿐이며 급하지 않은 선택 사항이다 |
