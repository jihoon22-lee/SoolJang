...
| 완료된 Task | **Task 1 ~ Task 17, Task 20** (Task 18·19 는 검색 API 미해결로 건너뜀) |
| 다음 착수 Task | **Task 18 — 외부 소스 레지스트리** (검색 API 제공자 미해결로 여전히 차단, 아래 참조). 또는 Task 21(자체 통합 테스트)로 먼저 넘어가는 것도 검토할 수 있다 |
| 현재 브랜치 | `main` (Task 20 까지 머지 완료, 열린 PR 없음) |
| 진행 중 잔여 항목 | 없음 |
...
피벗·시계열·저장뷰를 만들었다. Task 21(자체 통합 테스트)도 Task 18·19 없이 착수할 수
있다(§3 의존 관계 다이어그램 참조) — 검색 API 가 풀리기 전까지 다음 후보다.
...
| 20 | 통계 v2 — 커스텀 피벗과 취향 분석 | ✅ | `feature/stats-v2` | (이 PR) |
