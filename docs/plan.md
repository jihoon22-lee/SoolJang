...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-09-07 — B01/B02/B03/B04/B05/B06/B08 CI 통과·머지. B07 전체 통합 검사 완료·PR 제출, B09 수용 검증 진행 |
...
| 다음 착수 Task | **B09/WP13 #131 최종 통합 수용**. B07 #128 PR 검사·머지 후 복구·성능·브라우저 결과 통합 |
| 현재 브랜치 | `feature/v170-price-watch` — B07. 실제 재개 시 Git/열린 PR 및 분리 worktree 상태 확인 |
| 진행 중 잔여 항목 | B01/B02/B03/B04/B05/B06/B08 머지(B05 #139). B07 목표가/감시·푸시 구현과 전체 통합 검사 완료, B09 통합 수용 후 릴리스·배포. 사용자 승인: 필수 CI 통과 후 자율 머지, 마일스톤 완료 후 릴리스·배포(2026-09-07). 유료 계약/신규 키 발급은 자동 수행하지 않음 |
...
권한과 실검증의 제약은 담당 WP에서 확인한다. 지침 준비 자체에 남은 사용자 결정은 없다. B06의 최초 v1.6→v1.7 교체 전에는 기존 탭의 입력을 저장해야 하며 구버전 메모리 draft의 소급 복구는 보장하지 않는다. B02는 운영 실데이터 변경 없이 격리 복원까지 검증했으며 실제 배포의 백업·전환은 승인된 B09에서 실행한다. 사용자가 Exa 키 사용을 승인해 ~/.bashrc에서 읽고 비생성 연결 확인1회를 성공했다. 네이버·Brave와 국내 판매 자료의 재사용 허용은 아직 미확정이다. 키 값은 채팅에 요구하지 않는다. 소스별 이용/표시/보관 권한과
실제 수용 대상은 [소스 원장](plans/v1.7.0/sources.md)에 미확정으로 구분한다.
