...
| 진행 중 잔여 항목 | B01·B02 머지. B03 #136 머지. B04 전체 검사·실브라우저·복구 완료/PR CI 예정, B06 #135 통합 CI 진행. B05 탐색·B07 목표가·B08 데이터·재고 구현 중. 운영 수용은 B09에 남음. 소스별 계약·실연결과 B04 매칭 개선은 별도 수용. B02/B06 및 B03~B09를 이어간다. 사용자 승인: 필수 CI 통과 후 자율 머지, 마일스톤 완료 후 릴리스·배포(2026-09-07). 유료 계약/신규 키 발급은 자동 수행하지 않음 |
...
권한과 실검증의 제약은 담당 WP에서 확인한다. 지침 준비 자체에 남은 사용자 결정은 없다. B02는 운영 실데이터 변경 없이 격리 복원까지 검증했으며 실제 배포의 백업·전환은 승인된 B09에서 실행한다. 현재 네이버·Brave·Exa 기존 계정/앱 등록 여부를
