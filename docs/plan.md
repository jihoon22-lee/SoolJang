...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-09-07 — #116/#132 머지 완료. B01 #133·B02 #134 CI 통과·머지. B06 동기화 구현·실브라우저 검증 완료, B03 #136 머지, B04·B07·B08 병행 |
...
| 현재 브랜치 | `feature/v170-sync` — B06. 실제 재개 시 Git/열린 PR 및 분리 worktree 상태 확인 |
| 진행 중 잔여 항목 | B01·B02 머지. B06 구현·실브라우저 검증 완료, 통합 전체 검사 통과, B03 통합 후 CI 재검증. B03 #136 머지. B04 복수 가격 검증 및 B05 관심/탐색 구현 준비 중. 운영 전환 수용은 B09에 남음. 소스별 계약·실연결과 B04 매칭 개선은 별도 수용. B02/B06 및 B03~B09를 이어간다. 사용자 승인: 필수 CI 통과 후 자율 머지, 마일스톤 완료 후 릴리스·배포(2026-09-07). 유료 계약/신규 키 발급은 자동 수행하지 않음 |
...
| B03 / WP05 | 🟡 #136 CI 통과·머지, 실인증 수용 남음 | [기록](../workthrough/2026-09-07-v170-b03-connections.md) |
...
권한과 실검증의 제약은 담당 WP에서 확인한다. 지침 준비 자체에 남은 사용자 결정은 없다. B06의 최초 v1.6→v1.7 교체 전에는 기존 탭의 입력을 저장해야 하며 구버전 메모리 draft의 소급 복구는 보장하지 않는다. B02는 운영 실데이터 변경 없이 격리 복원까지 검증했으며 실제 배포의 백업·전환은 승인된 B09에서 실행한다. 현재 네이버·Brave·Exa 기존 계정/앱 등록 여부를
질문했으며 답변은 아직 없다. 키 값은 채팅에 요구하지 않는다. 소스별 이용/표시/보관 권한과
