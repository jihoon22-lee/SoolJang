...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-09-07 — v1.7.1 가격 공란 0원·등록 빈티지 보존 패치. 하나의 PR로 검증·머지·릴리스·배포하는 사용자 승인 |
| 완료된 Task | **Task 1 ~ Task 17, Task 20 ~ Task 34**(Task 24~28 은 v1.1.x 실사용 피드백 개선, Task 29 는 접근성·릴리스 가드, Task 30~33 은 백로그 정리·실사용 개선, Task 34 는 외부 정보 조회 v2 — PR1~PR7). Task 18 은 `adapter` 전략 + JSON 모드로 확장, 외부 소스 7곳 중 1곳(데일리샷) 실등록. Q5(웹 푸시 채널) 는 웹 푸시로 결정됨 — Task 19 본 사양(시세 이력·목표가 알림)은 B07 #140으로 구현·검증 완료(실가격·기기 수용 후속). Task 23(첫 릴리스·배포)은 완료 |
| 현재 패치 / 후속 | **v1.7.1 가격·빈티지 정정**. 기능 확대 후속 #142/#143은 별도 유지 |
| 패치 브랜치 | `feature/purchase-price-vintage` — 하나의 PR. 실제 재개 시 Git/열린 PR 확인 |
| 진행 중 잔여 항목 | B01~B08 머지(B05 #139, B07 #140). B09 Python1322/91.22%·웹634/81.09%, 실제 복구·401/SW·Docker no-recreate 통과. 실가격/실기 푸시 수용은 사용자 승인 후속 #142/#143. 운영 백업/실복원·0018 전환·HTTPS/실브라우저·설정/기록 보존 확인 완료. 사용자 승인: 필수 CI 통과 후 자율 머지, 마일스톤 완료 후 릴리스·배포(2026-09-07). 유료 계약/신규 키 발급은 자동 수행하지 않음 |
| 버전 상태 | 코드·패키지 `1.7.1`, schema `0018_price_watch` 유지. [패치 검증](../workthrough/2026-09-07-v171-price-vintage.md)·[릴리스/배포 원장](https://github.com/jihoon22-lee/SoolJang/releases/tag/v1.7.1) |

- 완료된 WP #119/#120/#122/#123/#129/#130은 실행 근거와 함께 종료했다.
  구현·검증 PR9개 머지와 GitHub closed 이슈 비율·최종 수용은 다른 지표다.
...
- 구매 가격 공란을 0원으로 해석해 Python·SQL·오프라인 평균/원가/품질 표시를 일치시킨다.
- 등록 빈티지를 우선하고 외부 관측의 상세 식별 근거를 저장·재계산에서 보존한다.
- 하나의 PR에 버전·문서·회귀 테스트를 포함한다. 실제 결과는 [패치 기록](../workthrough/2026-09-07-v171-price-vintage.md)과 릴리스 노트를 따른다.
...
| B01 / WP01·WP04 | 완료: #133 기반/안전 요청, 실가격 후속#142 | [작업 기록](../workthrough/2026-09-07-v170-b01-foundation.md) |
| B02 / WP03 | 완료: #134 복구/readiness, 운영1.7.0 전환 확인 | [작업 기록](../workthrough/2026-09-07-v170-b02-recovery.md) |
| B06 / WP02 | 완료: #135 동기화/PWA, B09와 운영 교체 확인 | [작업 기록](../workthrough/2026-09-07-v170-sync.md) |
| B03 / WP05 | 완료: #136 연결 관리·실제 Exa, 선택형 제공자 제한 명시 | [기록](../workthrough/2026-09-07-v170-b03-connections.md) |
| B04 / WP06·07 | 완료(승인 범위): #137 구현, 실제 국내2출처는#142 | [기록](../workthrough/2026-09-07-v170-b04-matching-offers.md) |
| B08 / WP11·12 | 완료: #138 정리/재고, B09·운영 보존 확인 | [기록](../workthrough/2026-09-07-v170-b08-data-inventory.md) |
| B05 / WP08·09 | 완료: #139 실제 Exa·앱 탐색, 국내 가격은#142 | [기록](../workthrough/2026-09-07-v170-b05-discovery.md) |
| B07 / WP10 | 완료(승인 범위): #140 구현, 실제 기기 수신은#143 | [기록](../workthrough/2026-09-07-v170-b07-price-watch.md) |
| B09 / WP13 | 완료: #141 통합·#144 릴리스·#145 운영 증거, #142/#143 승인 이연 | [수용 원장](plans/v1.7.0/acceptance.md), [기록](../workthrough/2026-09-07-v170-b09-acceptance.md) |
...
### v1.7.0 B01 결정 (D197~D199, 2026-09-07)

| # | 결정 | 근거 |
|---|---|---|
| D197 | 요청 전 DNS/IP 검증과 실제 TCP 목적지를 묶고 원래 TLS/Host를 유지한다. 각 hop 요청 예산은 별도 DB 트랜잭션으로 예약한다 | redirect 사후 검사·메모리 카운터의 SSRF/재시작·공유 연결 한도 공백을 닫음. [계약](specs/v1.7.0-discovery.md) |
| D198 | 설정 revision과 성공/실패 유형을 probe/캐시와 연결하고 과거 정상은 재검증 필요로 표시한다 | 등록·검증·활성을 혼동하지 않음. 기존 키·고정·사용자 override 보존 |
| D199 | 합성 정답셋 v1과 성능 기준선을 개선 전에 고정하고 실제 소스 계약/권리·미검증 상태를 분리한다 | [기준선](plans/v1.7.0/baseline.md)의 자동 오탐2/다국어 누락4는 B04에서 해결. [소스 원장](plans/v1.7.0/sources.md)의 조건부를 실연결로 세지 않음 |

---

### v1.7.0 B02 결정 (D200, 2026-09-07)
...
### v1.7.0 B06 결정 (D201, 2026-09-07)

| 번호 | 결정 | 근거 |
|---|---|---|
| D201 | 계정별 IndexedDB·명령별 legacy 이전 기록·200건 전송과 서버 소유자 확인을 적용한다. 온라인/동기화 쓰기는 같은 행 잠금을 사용하고 SW 업데이트는 dirty/outbox 상태를 따른다 | 독립 로그인 세션에서 잔량·제품 변경 유실을 실제 재현 후 수정. 두 탭·오프라인 재시작·실제 SW 교체 검증. 최초 구버전 메모리 입력은 저장 후 교체 필요 |
...
### v1.7.0 B04 결정 (D203, 2026-09-07)

| 번호 | 결정 | 근거 |
|---|---|---|
| D203 | 원래 판본/규격을 검색어와 분리하고 제품 고정 아래 복수 판매 조건·실제 관측을 저장한다. 장기 가격 저장은 소스별 허용 확인 후 켠다 | 합성 오탐0·정답후보13/13, 세 판매처/다른용량·캐시/실패·pin이전·0015실제복구·모바일비교 통과. 실제 국내2출처 수용은 별도 |

### v1.7.0 B05 초기 계약 (D204, 2026-09-07)

| 번호 | 결정 | 근거 |
|---|---|---|
| D204 | 초기 관심은 구매와 별도 UUID로 저장하며 기존 외부 관측/캐시에 같은 관심 ID를 연결한다. 반복 저장은 request_id로 멱등 처리한다 | 관심 CRUD·합계 제외·revision·0016 왕복/drift 집중 검증. B07/B08에 공유. Exa 환경 키 사용을 승인받아 비생성 연결 요청1회 성공 |

### v1.7.0 B08 결정 (D205, 2026-09-07)

| # | 결정 | 이유·검증 |
|---|---|---|
| D205 | 정리는 영향 미리보기와 재검증 후 원자적으로 확정하며 관심 구매 전환은 원래 Interest ID·멱등 receipt를 보존한다. 위치/병 QR/실사는 기존 구매·재고 상태를 자동 변경하지 않는다 | Python1170/30skip/92.15%, 웹591/branch80.57%, 0017왕복/drift/사실 보존, 실제 Chromium10흐름과 QR해독. 후속 잠금 순서 수정은 서로 다른 로그인 세션의 동시 병합/온라인·sync 수정 포함32검사 통과 |

### v1.7.0 B05 결정 (D206, 2026-09-07)

| # | 결정 | 이유·검증 |
|---|---|---|
| D206 | 새 탐색은 선택한 최대4개의 검색/전문 소스에서 비생성 자료를 조회하고, 정확한 기존 pin 문맥을 관심 ID로 보존한다. 원문·증거는 영속 보관하지 않고 명시 선택 필드만 revision 검증 후 적용한다 | 605웹/14합성브라우저 흐름·실제API/PG/Exa브라우저, 송신 대기 취소·중첩가드·보관권한/pin 변경·인코딩 키 반사 회귀. 국내2출처 실제 수용은 사용자 결정/허용 근거 대기 |

### v1.7.0 B07 결정 (D207, 2026-09-07)

| # | 결정 | 이유·검증 |
|---|---|---|
| D207 | 가격 감시는 공유 Interest ID와 실제 관측의 판매 조건만 사용한다. 정기 조회/푸시는 별도 동의이며 일정·lease·알림·전송 원장을 영속화한다 | 새 관측/목표가/중복worker/중지경쟁/불확실전송 재시도금지 및0018이전 검증. B05통합Python1297/91.17%, 웹628/81.01%; 실제DB/API/worker브라우저는 합성 HTTP/PushAPI이며 실제수신·운영수용은 B09 |

### v1.7.0 B09 결정 (D208, 2026-09-07)

| # | 결정 | 근거·검증 |
|---|---|---|
| D208 | 고정 합성 성능 규모와 최신 운영 합계를 구분하고 R/S별 실제 검증 층을 수용 원장에 연결한다. 첨부 인증 열람·새 Docker 볼륨 소유권과 선택형 Vite polling을 보강한다 | 최종 전체 Python1322/91.22%, 웹634/81.09%, 실제0018복원·401응답유실·새폼SW·컨테이너no-recreate 통과. 국내2가격출처/실푸시/운영전환은 미실행으로 유지. [B09 기록](../workthrough/2026-09-07-v170-b09-acceptance.md) |

### v1.7.0 릴리스 결정 (D209, 2026-09-07)

| # | 결정 | 근거·검증 |
|---|---|---|
| D209 | 사용자 승인에 따라 국내 가격 출처 확대·실제 수용은 #142, 실제 기기 푸시 수용은 #143으로 이연하고 구현된1.7.0을 릴리스·배포한다 | 열린 WP를 코드/실행 근거와 재대조했다. 기존 키/기록 보존·최종 CI·실제 운영 백업/격리 복원·명시 전환은 이연하지 않는다. [릴리스 기록](../workthrough/2026-09-07-v170-release.md) |

### v1.7.0 운영 검증 (D210, 2026-09-07)

| # | 결정 | 근거·검증 |
|---|---|---|
| D210 | 승인 이연만 후속으로 남기고1.7.0 운영 수용을 완료한다 | release SHA f1c5bbc·API/web image label/digest·0018 일치, 직전/전환 후 백업·실제 복원·원래 컬럼 보존·DPAPI 복구·HTTPS/실브라우저·Exa 실탐색 통과. [배포 기록](../workthrough/2026-09-07-v170-release.md) |

### v1.7.1 사용자 정정 (D211, 2026-09-07)

| 결정 | 내용 | 근거 |
|---|---|---|
| D211 | 구매 가격 공란은 선물·포인트 취득의0원으로 계산하며 제품/전체 평균 분모에도 포함한다. 등록 빈티지를 상품명보다 우선하고 외부 관측 근거를 보존한다 | 사용자가 가격 공란/빈티지 해석을 명시적으로 정정하고 하나의 PR·v1.7.1 문서·릴리스·배포까지 승인. 기존 공란을 미상으로 분류하던 해석을 대체한다. [패치 기록](../workthrough/2026-09-07-v171-price-vintage.md) |
...
B06의 최초 v1.6→v1.7 교체 전에는 기존 탭의 입력을 저장해야 하며 구버전 메모리 draft의
소급 복구는 보장하지 않는다. 최종0018 격리 복구·Docker no-recreate는 통과했고 운영 직전/전환 후
백업과 실제 복원·전환을 완료했다. Exa는 사용자 환경 키로 인증·6주종·실제 앱/브라우저를
검증했으며 키 값은 채팅에 요구하지 않는다. **R11의 국내 가격2출처는 #142, 기기 푸시 수용은 #143으로 이연하고 먼저 릴리스·배포하라는 사용자 결정을 받았다(2026-09-07)**. 기존 감시/실푸시·운영 수용을
자동 통과시키지 않는다. [수용 원장](plans/v1.7.0/acceptance.md)과
[소스 원장](plans/v1.7.0/sources.md)에 미확정 범위를 남긴다.
