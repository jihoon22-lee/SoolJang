...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-09-07 — #116/#132 머지 완료. B01 외부 요청/상태/평가 기반 구현·로컬 검증 완료, B02 복구·B06 동기화 병행 |
...
| 다음 착수 Task | **B03/WP05 #123 외부 연결 관리**, B01 최소 계약 기반. B02 복구 및 B06 동기화는 독립 구현/검증 후 통합 |
| 현재 브랜치 | `feature/v170-foundation` — B01. 실제 재개 시 Git/열린 PR 및 분리 worktree 상태 확인 |
| 진행 중 잔여 항목 | B01은 코드·로컬 검증 완료/PR CI 전. 소스별 계약·실연결과 B04 매칭 개선은 별도 수용. B02/B06 및 B03~B09를 이어간다. 사용자 승인: 필수 CI 통과 후 자율 머지, 마일스톤 완료 후 릴리스·배포(2026-09-07). 유료 계약/신규 키 발급은 자동 수행하지 않음 |
...
| B01 / WP01·WP04 | 🟡 구현·로컬 검증 완료, CI/리뷰 예정 | [작업 기록](../workthrough/2026-09-07-v170-b01-foundation.md) |
| B02 / WP03 | 🟡 별도 worktree에서 복구 구현·합성 실검증, 통합 예정 | [로드맵](roadmap/v1.7.0.md) |
...
권한과 실검증의 제약은 담당 WP에서 확인한다. 지침 준비 자체에 남은 사용자 결정은 없다. 현재 네이버·Brave·Exa 기존 계정/앱 등록 여부를
