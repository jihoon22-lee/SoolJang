...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-09-07 — B01~B09 CI 통과·머지. 사용자 승인으로 실가격/기기 푸시 수용을 #142/#143에 이연하고1.7.0 릴리스·배포 진행 |
...
| 다음 착수 Task | **v1.7.0 릴리스·배포**. 버전 PR/CI→태그/이미지→최신 운영 백업·복원→명시 전환 |
| 현재 브랜치 | `feature/v170-release` — 릴리스. 실제 재개 시 Git/열린 PR 및 분리 worktree 상태 확인 |
| 진행 중 잔여 항목 | B01~B08 머지(B05 #139, B07 #140). B09 Python1322/91.22%·웹634/81.09%, 실제 복구·401/SW·Docker no-recreate 통과. 실가격/실기 푸시 수용은 사용자 승인 후속 #142/#143. 운영 수용은 이번 배포에서 확인. 사용자 승인: 필수 CI 통과 후 자율 머지, 마일스톤 완료 후 릴리스·배포(2026-09-07). 유료 계약/신규 키 발급은 자동 수행하지 않음 |
| 버전 상태 | 패키지 `1.7.0` 준비. 2026-09-07 운영 readonly 확인: API/web 이미지 `1.6.1`, health 정상, 실제 schema `0012_llm_rematch`. 새 버전 운영 변경은 아직 실행하지 않음 |
...
| B09 / WP13 | 🟡 최종 격리 검증·#141 CI/머지 완료, 승인 이연과 운영 전환 진행 | [수용 원장](plans/v1.7.0/acceptance.md), [기록](../workthrough/2026-09-07-v170-b09-acceptance.md) |
...
소급 복구는 보장하지 않는다. 최종0018 격리 복구·Docker no-recreate는 통과했고 운영 직전
백업·전환은 수용 후 실행한다. Exa는 사용자 환경 키로 인증·6주종·실제 앱/브라우저를
