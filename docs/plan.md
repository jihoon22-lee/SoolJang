...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-08-20 (**Task 34 PR4 완료** — 소스별 조회 시도(성공·실패 가리지 않음)를 `external_source_probe` 롤링 로그(최근 20개)에 남기고, 연속 실패 3회 이상이면 `failing`, 최신 시도가 실패·degraded 면 `degraded` 로 헬스를 계산한다. 샘플 제품명으로 캐시에 남기지 않고 테스트 조회하는 기능도 추가. D185~D186. 이전 갱신은 PR3 완료 — 표준 필드 스키마와 가격 비교 뷰(D182~D184)) |
...
| 다음 착수 Task | **Task 34 PR5 — 소스 프리셋 카탈로그와 `adapter_spec` v2**([실행 계획](plan-external-v2.md)). 데일리샷 `adapter_spec` 원문이 확보돼 있어 착수를 막는 것이 없다 |
| 현재 브랜치 | `feat/external-source-health`(Task 34 PR4, 머지 대기) |
| 진행 중 잔여 항목 | Task 34 PR5~PR7. 그 외엔 §9 백로그·§6 Q6 뿐이며 급하지 않은 선택 사항이다 |
