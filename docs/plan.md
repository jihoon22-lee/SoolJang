...
|---|---|
| 최종 갱신 | 2026-08-01 |
| 완료된 Task | **Task 1 ~ Task 10** |
| 다음 착수 Task | **Task 11 — 레거시 데이터 임포터** |
...
파서(Task 6)와 API(Task 9)가 준비됐다. 이제 실제 429행을 DB 에 넣는다.

1. 업로드 엔드포인트 — CP949 CSV 를 받아 `legacy.parse_sheet` 로 분석
2. **dry-run 미리보기** — 생성될 제품·규격·구매 건 수, 경고, 중복 후보를 먼저 보여준다.
   실제 적재 전에 사용자가 확인해야 한다
3. 컬럼 매핑 조정 UI (기본 매핑이 맞으면 그대로 진행)
4. 제품 자동 병합 — `normalized_name` + `vintage` + `abv` 로 판정
5. 구매처 개행 분할 시도. 총합이 `구매` 병수와 맞지 않으면 포기하고 단일 구매 건으로
   적재하되 원문을 `purchase.import_note` 에 보존한다
6. 병 레코드 초기화 — `소비` 만큼 `finished`, `미개봉` 만큼 `unopened`, `개봉` 만큼 `open`
7. 재실행 멱등성 — 같은 파일을 두 번 넣어도 중복이 생기지 않아야 한다
8. **[legacy-schema.md](legacy-schema.md) §5 기준값 전체 대조**
   (429행, 1,078/819/259/225/34병, ₩42,401,100, ₩36,495,447, 704,970ml, 구매처 82곳)
