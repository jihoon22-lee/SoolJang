...
| 다음 착수 Task | 없음 — Task 33 까지 계획된 작업이 전부 끝났다. 남은 건 §9 백로그(목표가 감시·읽기 전용 공유 링크)와 §6 Q6(공유 권한)·Q9(외부 소스 6곳·Task 19) 뿐이며 전부 사용자가 원하는 시점에 결정할 선택 사항이다 |
| 현재 브랜치 | `fix/stock-tier-priority-order`(Task 33 머지 후 v1.4.1 릴리스·재배포 진행 중) |
| 진행 중 잔여 항목 | v1.4.1 릴리스·재배포(태그 푸시 → GHCR 게시 → `docker compose pull && up -d`). 그 외엔 §9 백로그·§6 Q6·Q9 뿐이며 급하지 않은 선택 사항이다 |
