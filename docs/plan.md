...
| 최신 버전 | `v1.0.0` 릴리스 완료 후 실사용 피드백으로 상당한 기능이 추가돼(Task 24: 통계 차트, 탭 재구성, 주종 관리 개편, 외부 소스 JSON 어댑터 등) **`v1.1.0` 버전 범프 진행 중**(2026-08-06) — 태그 푸시로 정식 릴리스 예정. 이번엔 GHCR `read:packages` 스코프가 있어(Q8) `docker compose pull` 로 직접 받을 수 있다 |
