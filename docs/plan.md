...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-09-07 — #116/#132 머지 완료. B01 #133·B02 #134·B03 #136 머지. B04 가격 비교·복구·실브라우저/전체 검사 완료, B06 #135 통합 CI 진행 |
...
| 진행 중 잔여 항목 | B01·B02 머지. B03 #136 머지. B04 전체 검사·실브라우저·복구 완료/PR CI 예정, B06 #135 통합 CI 진행. B05 관심0016/최소API·Exa 실인증 완료, 탐색 서비스/UI 구현 중. B07 목표가·B08 데이터·재고 병행. 운영 수용은 B09에 남음. 소스별 계약·실연결과 B04 매칭 개선은 별도 수용. B02/B06 및 B03~B09를 이어간다. 사용자 승인: 필수 CI 통과 후 자율 머지, 마일스톤 완료 후 릴리스·배포(2026-09-07). 유료 계약/신규 키 발급은 자동 수행하지 않음 |
...
| B04 / WP06·07 | 🟡 전체 로컬·실브라우저·복구 통과, PR CI 예정 | [기록](../workthrough/2026-09-07-v170-b04-matching-offers.md) |
...
권한과 실검증의 제약은 담당 WP에서 확인한다. 지침 준비 자체에 남은 사용자 결정은 없다. B02는 운영 실데이터 변경 없이 격리 복원까지 검증했으며 실제 배포의 백업·전환은 승인된 B09에서 실행한다. 사용자가 Exa 환경 키 사용을 승인했고 새 셸에서 실제 연결1회 성공을 확인했다. 네이버·Brave 및 국내 판매 자료 재사용 허용은 아직 미확정이다. 키 값은 채팅에 요구하지 않는다. 소스별 이용/표시/보관 권한과
