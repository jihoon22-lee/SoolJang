...
| 다음 착수 Task | **Task 34 1단계 — 외부 정보 조회 v2**(PR1~PR4, [즉시 착수 계획](plan-external-v2.md)). 사용자 입력 없이 진행 가능하고 정확도 문제의 실질적 해결이 전부 이 넷 안에 있다. 2단계(PR5~PR10)는 [보류 계획](plan-external-v2-deferred.md) 에 차단 해제 조건과 함께 분리해 뒀다. `v1.4.1` 릴리스·재배포를 먼저 마친 뒤 PR1 부터 시작한다 |
...
| 34 | 외부 정보 조회 v2 — 매칭 정확도 개선과 소스 확장. **1단계 PR1~PR4**([즉시 착수 계획](plan-external-v2.md), 차단 없음) + **2단계 PR5~PR10**([보류 계획](plan-external-v2-deferred.md), 사용자 입력 필요) | ⬜ | `feat/external-*` (PR별 분리) | |
...
### ⬜ Task 34 — 외부 정보 조회 v2 (매칭 정확도 개선과 소스 확장)

- **상세 계획 — 범위별로 두 문서로 나눴다**
  - [plan-external-v2.md](plan-external-v2.md) — **즉시 착수 범위(1단계, PR1~PR4)**.
    사용자 입력 없이 착수해 머지까지 갈 수 있는 것만. **현재 작업 대상**
  - [plan-external-v2-deferred.md](plan-external-v2-deferred.md) — **보류 범위(2단계,
    PR5~PR10)**. 답변·자격 증명·사이트 응답 샘플이 있어야 진행되는 것. 계획과
    체크리스트만 남기고 세부 설계는 착수 시점으로 미뤘다
...
- **1단계 (차단 없음, `v1.5.0`)**: PR1 후보 노출·매칭 고정 / PR2 매칭 점수 재작성·질의
  확장 / PR3 표준 필드 스키마·가격 비교 뷰 / PR4 소스 헬스 체크.
  **현황 진단 10건 중 9건을 이 넷이 덮는다** — 정확도 문제의 실질적 해결이 전부 여기
  있고, 기존 코드와 이미 등록된 데일리샷 소스만으로 구현·검증이 끝난다
- **2단계 (차단 있음, `v1.6.0`~`v1.7.0`)**: PR5 프리셋 카탈로그·`adapter_spec` v2 /
  PR6 LLM 재판정 / PR7 제외 키워드 / PR8 공식 API 소스(네이버 쇼핑·Untappd) / PR9 평점
  소스(Whiskybase·RateBeer·BeerAdvocate) / PR10 국내 몰(이마트몰·트레이더스·코스트코)
- **차단 해제 조건**: PR5 는 현재 등록된 데일리샷 `adapter_spec` 원문 1건(저장소에 없고
  사용자 DB 에만 있다), PR6·PR7 은 답 1건씩(기본값·초안으로 착수 가능), PR8~PR10 은
  **개발 샌드박스에서 외부 도메인 접속이 안 되므로**(Task 18 절 참조) 사용자 환경에서
  robots.txt·요청·응답 샘플을 받는 협업 루프가 필요하다
- **열린 질문**: Q10(LLM 비용 상한)·Q11(API 키 발급 주체)·Q12(robots·ToS 판단
  기준)·Q13(편의점 3사 제외 동의). 진행을 실제로 막는 것은 **Q11 뿐**이고, 나머지는
  기본값·초안으로 착수한 뒤 확정할 수 있다 — 상세는 보류 계획서 §1·§2
...
| Q9 | Task 19(판매처 시세 이력) 를 지금 시작할지, 계속 미룰지 | **부분 진행(2026-08-05, [PR #56](https://github.com/jihoon22-lee/SoolJang/pull/56))** — 데일리샷을 실제 등록했다(Q3 참조). 등록 과정에서 `adapter_spec` 에 JSON 모드를 새로 추가하고(SPA+API 사이트 대응) 이름 유사도 매칭의 실제 오탐(증류소 접두사 혼동)도 발견해 고쳤다 — 상세는 §5 D147~D148. 나머지 6곳(이마트·트레이더스·코스트코·CU·GS25·emart24)은 아직이고, Task 19 본 사양(시세 이력 차트·목표가 알림)은 여전히 미착수다. `search` 전략(Q2)은 D167 에서 폐기로 결정됐다. **2026-08-19 사용자 보고로 Task 34 착수** — 정확도 개선(매칭 고정·점수 재작성)과 소스 확장(프리셋·사이트)을 PR 10개로 나눠 진행한다 — 차단 없는 [1단계](plan-external-v2.md)(PR1~PR4)와 사용자 입력이 필요한 [2단계](plan-external-v2-deferred.md)(PR5~PR10)로 분리 | Task 34 로 진행 중 |
