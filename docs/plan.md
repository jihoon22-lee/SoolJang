...
- **카테고리 관리 API (§5-D24)**: `POST /categories/{id}:reparent`(서브트리 동반 이동,
  순환 검사), `POST /categories:reorder`, `POST /categories/{id}:merge`,
  `POST /categories:reset-seed`. 삭제는 `strategy=promote_children|reassign` 을 지원한다
- **테스트**: 필터 조합, 한글 부분 일치, 정렬 안정성(id tie-breaker), 커서 경계,
  에러 응답 형식, 순환 이동 거부, 하위·소속 제품이 있는 카테고리 삭제 거부
