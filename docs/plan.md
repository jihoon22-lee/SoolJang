...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-08-20 (**Task 34 PR6 완료** — 애매 구간(점수 0.5~0.85) 매칭을 LLM 으로 재판정하는 보조 기능(`infrastructure/external/match_llm.py`)을 추가했다. 기본 꺼짐, 사용자가 설정 화면에서 "LLM 매칭 보조"를 켜야만 동작한다. 자동 고정하지 않는다 — "LLM 추천" 배지만 붙이고 고정은 여전히 사용자가 누른다. 비용 가드 3중(조회당 1회, 같은 (소스, 제품) 24시간 dedup, 월 호출 상한 — 설정값, 기본 200회). D191~D192. 이전 갱신은 PR5 완료 — 소스 프리셋 카탈로그와 adapter_spec v2(D187~D190)) |
...
| 다음 착수 Task | **Task 34 PR7 — 제외 키워드**([실행 계획](plan-external-v2.md)). 초안 목록은 데일리샷 실사용 근거로만 채운다 |
| 현재 브랜치 | `main`(Task 34 PR6, 머지 완료) |
| 진행 중 잔여 항목 | Task 34 PR7. 그 외엔 §9 백로그·§6 Q6 뿐이며 급하지 않은 선택 사항이다 |
