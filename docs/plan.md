...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-08-20 (**Task 34 PR1 완료** — 외부 조회 결과에 매칭 정보(무엇에 어느 정도 확신으로 매칭됐는지)와 후보를 함께 노출하고, 사용자가 후보를 골라 고정하면 이후 조회가 그 상품을 그대로 보게 했다. D175~D178. 이전 갱신은 2026-08-16 **Task 33 완료** — Task 32 회귀 수정. 재고 우선 정렬 순위가 사용자 의도와 반대(미개봉 우선)로 구현돼 있었던 것을 "개봉 있음 > 미개봉만 있음 > 재고 없음"으로 뒤집었다(D174). `stockTier()` 분기 순서만 교체 + 체크박스 라벨·테스트 동기화) |
...
| 다음 착수 Task | **Task 34 PR2 — 매칭 점수 재작성과 질의 확장**([실행 계획](plan-external-v2.md)). PR1 이 "확신 없으면 물어본다" 는 바닥을 깔았으니, PR2 가 자동 정확도 자체를 올린다(토큰 집합 + 용량·연수·빈티지·도수 하드 제약, 접두사 게이트 제거) |
| 현재 브랜치 | `feat/external-match-pin`(Task 34 PR1) |
| 진행 중 잔여 항목 | Task 34 PR2~PR7. 그 외엔 §9 백로그·§6 Q6 뿐이며 급하지 않은 선택 사항이다 |
| 최신 버전 | **`v1.4.1`**(2026-08-16, 태그·GHCR 이미지 게시 완료) — Task 33(재고 우선 정렬 순위 수정) 반영. 홈 PC 에 `sooljang-api:1.4.1`·`sooljang-web:1.4.1` 로 배포돼 실행 중임을 확인했다(2026-08-20). Task 34 는 `v1.5.0` 으로 나간다 |
...
| 34 | 외부 정보 조회 v2 — 매칭 정확도 개선과 소스 등록 개선(PR1~PR7, [실행 계획](plan-external-v2.md)) | 🟡 PR1~PR3 완료 | `feat/external-*` (PR별 분리) | |
...
| # | 결정 | 근거 |
|---|---|---|
| D182 | 조회 결과를 표준 키(`price_krw`·`rating`·`rating_scale`·`volume_ml`·`in_stock` 등)로 정규화하고, **표준 키가 아닌 값은 버리지 않고 `extra` 에 담는다** | 소스가 하나일 때는 자유 dict 로도 됐지만, 여럿이 되면 서로 다른 이름의 값을 나열하는 카드가 늘어날 뿐 비교가 불가능하다(진단 8번). 실제로 등록된 데일리샷 스펙은 `가격`·`평점`·`리뷰수` 라는 한글 키를 쓴다 — `extra` 폴백이 없으면 이 PR 이 기존 소스를 깨뜨린다. 값 변환도 함께 한다(`"89,000원"` → 89000): 안 하면 최저가 비교가 문자열 비교가 된다 |
| D183 | `rating_normalized`(0~5 환산)와 100ml당 가격은 **API 응답 조립 시점에 계산**하고 DB 에 저장하지 않는다 | §8 절대 규칙 6(파생값을 DB 에 저장하지 않는다)의 직접 적용이다. 100ml당 가격은 이 앱이 이미 쓰는 `price_per_100ml` 파생 지표와 같은 단위라, 내 실평단가와 나란히 놓고 "내가 산 가격보다 12% 비쌈" 을 볼 수 있다 — 그게 이 기능의 실질 가치다 |
| D184 | 스냅샷에 `version` 을 넣고 `SNAPSHOT_VERSION` 보다 낮은 캐시 행은 **TTL 과 무관하게 stale 로 취급**한다. 데이터 마이그레이션은 하지 않는다 | 캐시는 정의상 언제든 버려도 되는 데이터다 — 마이그레이션 스크립트를 쓸 이유가 없고, 다음 조회에서 자연스럽게 새 모양으로 교체된다. 구조를 바꿀 때마다 같은 방식으로 버전만 올리면 된다 |
