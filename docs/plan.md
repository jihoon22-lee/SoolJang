...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-08-03 (Task 21 검증 마무리 + review 문서) |
| 완료된 Task | **Task 1 ~ Task 17, Task 20, Task 21, Task 22**(Track 1~4, 10 PR). Task 18 은 `adapter` 전략만 부분 완료. Task 21 은 모바일 실기기 검증만 환경 제약으로 배포 후로 이연 |
| 다음 착수 Task | `feature/self-review` PR 로 이 문서 갱신을 머지해 Task 21 을 공식 닫음 → 그 다음은 7개 판매처 사이트 `adapter_spec` 실제 등록(§1 Task 22 실행 요약 참조) 또는 Task 19/PR11(Q5 미해결로 여전히 차단) 판단, 혹은 Task 23 준비 |
| 현재 브랜치 | `main` (PR #26~#35 머지 완료, 열린 PR 없음) — 이 갱신은 `feature/self-review` 에서 진행 중 |
| 진행 중 잔여 항목 | 없음(Task 21 완료). 모바일 실기기 검증만 배포 후 수동 확인 필요 |
...
준비돼 있으니, 다음에 그 7개 사이트를 실제로 조사해 등록하는 것이 Task 19/PR11 착수
전에 남은 실질적 작업이다.
...
| Q3 | 초기 등록할 외부 소스 사이트 목록 | **답 받음(세션 로컬 plan, 2026-08-03) — 데일리샷·이마트·트레이더스·코스트코·CU·GS25·emart24.** 단 실제 `adapter_spec`(CSS 셀렉터) 등록은 아직 안 했다 — 사이트별 HTML 구조 조사가 필요한 별도 작업. 레지스트리 UI(`#sources`)는 PR9 에서 이미 준비됐다 | Task 19/PR11 착수 전 |
