...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-08-20 (**Task 34 PR3 완료** — 소스마다 제각각인 `fields` 자유 dict 를 표준 키(`price_krw`·`rating`·`volume_ml` 등)로 분류하고, 100ml당 가격·정규화 평점 같은 파생값을 응답 조립 시점에 계산해(절대 규칙 6, 저장 안 함) 소스별 카드 나열을 비교 가능한 표 하나로 바꿨다. 최저가 배지·내 실평단가 대비 델타 포함. D182~D184. 이전 갱신은 PR1·PR2 완료 — 조회 결과에 매칭 정보와 후보를 함께 노출하고 사용자가 고정할 수 있게 했고(D175~D178), 매칭 점수를 토큰 집합 + 하드 제약으로 재작성했다(D179~D181)) |
...
| 다음 착수 Task | **Task 34 PR4 — 소스 헬스 체크**([실행 계획](plan-external-v2.md)). 사용자 입력이 필요 없다 — 소스별 성공/실패 이력을 롤링 로그로 남겨, 소스를 늘리기 전에 무엇이 깨졌는지 볼 수 있게 한다 |
| 현재 브랜치 | `feat/external-normalized-fields`(Task 34 PR3, 머지 대기) |
| 진행 중 잔여 항목 | Task 34 PR4~PR7. 그 외엔 §9 백로그·§6 Q6 뿐이며 급하지 않은 선택 사항이다 |
| 최신 버전 | **`v1.4.1`**(2026-08-16, 태그·GHCR 이미지 게시 완료) — Task 33(재고 우선 정렬 순위 수정) 반영. 홈 PC 에 `sooljang-api:1.4.1`·`sooljang-web:1.4.1` 로 배포돼 실행 중임을 확인했다(2026-08-20). Task 34 는 `v1.5.0` 으로 나간다(PR1~PR4) |
