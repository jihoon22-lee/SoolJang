...
| 다음 착수 Task | **Task 34 — 외부 정보 조회 v2**(정확도 개선 + 소스 확장, PR 9개). 상세 실행 계획은 [plan-external-v2.md](plan-external-v2.md) 에 있다. `v1.4.1` 릴리스·재배포를 먼저 마친 뒤 PR1 부터 시작한다 |
...
| 34 | 외부 정보 조회 v2 — 매칭 정확도 개선과 소스 확장(PR 9개, [상세 계획](plan-external-v2.md)) | ⬜ | `feat/external-*` (PR별 분리) | |
...
- **사양·상세 계획**: [plan-external-v2.md](plan-external-v2.md) — PR 9개의 설계·파일
  목록·테스트 케이스·완료 조건을 전부 담았다. 이 절은 요약만 둔다
...
- **방향**: 정확도의 바닥을 "확신 없으면 물어본다 + 한 번 고치면 영구히 맞는다"로 먼저
  깔고(PR1), 그 위에서 자동 정확도를 올린다(PR2). 소스를 늘리기 전에 필드를 표준화해
  비교 가능하게 만들고(PR3), 프리셋으로 등록 비용을 낮춘 뒤(PR4) 사이트를 붙인다(PR5~7)
- **PR 구성**: PR1 후보 노출·매칭 고정 / PR2 매칭 점수 재작성·질의 확장 / PR3 표준 필드
  스키마·가격 비교 뷰 / PR4 소스 프리셋 카탈로그·`adapter_spec` v2 / PR5 공식 API 소스
  (네이버 쇼핑·Untappd) / PR6 평점 소스(Whiskybase·RateBeer·BeerAdvocate) / PR7 국내 몰
  (이마트몰·트레이더스·코스트코) / PR8 애매 구간 LLM 재판정 / PR9a 소스 헬스 체크 /
  PR9b 제외 키워드
- **착수 차단 분석**(상세 계획 §7): **PR1·PR2·PR3·PR9a 는 사용자 입력 없이 착수해
  머지까지 갈 수 있고, 정확도 문제의 실질적 해결이 전부 이 묶음 안에 있다.** PR4 는
  현재 등록된 데일리샷 `adapter_spec` 원문 1건(저장소에 없고 사용자 DB 에만 있다)이
  필요하다. PR8·PR9b 는 답이 필요하지만 기본값·초안으로 착수할 수 있다. PR5~7 만
  사용자 협업 없이는 완료할 수 없다
- **선행 조건**: `v1.4.1` 릴리스·재배포 완료. PR5~7 은 **개발 샌드박스에서 외부 도메인
  접속이 안 되므로**(Task 18 절 참조) 사용자 환경에서 요청·응답 샘플을 받아 진행하는
  협업 루프가 필요하다 — 절차는 상세 계획 §PR5~7 공통에 정리했다
- **열린 질문**: Q10(LLM 비용 상한)·Q11(API 키 발급 주체)·Q12(robots·ToS 판단
  기준)·Q13(편의점 3사 제외 동의) — 상세 계획 §6. 이 중 진행을 실제로 막는 것은
  Q11 뿐이고, 나머지는 기본값·초안으로 착수한 뒤 확정할 수 있다
