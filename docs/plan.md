...
| 항목 | 값 |
|---|---|
| 최종 갱신 | 2026-08-03 (Task 21·22 완료, 개발 샌드박스 인터넷 제약 기록) |
...
| 다음 착수 Task | **이 개발 샌드박스에서 자동으로 이어갈 작업이 없다** — 남은 두 항목(7개 판매처 사이트 `adapter_spec` 등록, Task 19/PR11)이 각각 실제 인터넷 접속과 사용자 판단(Q5, 실사용 확인)을 필요로 한다. 실제 인터넷이 되는 환경에서 사이트 조사 → `#sources` 등록 → 실사용 확인 → PR11 착수 여부 판단 순. 그 전까지는 Task 23(릴리스) 준비를 검토할 수 있다 |
| 현재 브랜치 | `main` (PR #26~#36 머지 완료, 열린 PR 없음) |
| 진행 중 잔여 항목 | 없음. 모바일 실기기 검증·7개 사이트 조사 모두 이 샌드박스 밖에서 해야 한다(환경 제약) |
