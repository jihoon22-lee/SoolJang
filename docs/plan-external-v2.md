...
**✅ 완료(2026-08-19)**. 설계 그대로 구현했다 — 매칭은 `external_url` 동등 비교로 고정하고
(`key` 필드는 값을 안 채운 채 스키마만 마련해 둠), 후보 목록은 접두사 게이트 이전 **전체**
후보에서 뽑는다. 백엔드 47건(어댑터 9 + 애플리케이션/DB 12 + API 6, 기존 회귀 포함 전체
737건) · 프론트엔드 4건(전체 499건) 전부 통과, 커버리지 Python 91.70%/TS 91%(문·83%(분기)
— 게이트 통과. 상세는 §5 D175~D178.
...
- [x] 위 15개 테스트 통과, 커버리지 게이트 유지 (15개 대신 실제로는 더 세분화해 19개로 —
      어댑터 9 + 애플리케이션/DB 12 + API 6 = 27건. 전체 스위트 Python 737건, TS 499건
      전부 통과. 커버리지 Python 91.70%, TS 문 90.98%/분기 83.01%(게이트 80%))
- [x] `alembic upgrade head` / `downgrade -1` 왕복 성공 (로컬 Postgres 로 실측 검증 —
      `env.py` 를 거치지 않는 방식. 상세는 커밋 메시지 참조)
...
| PR | 상태 | 차단 | 비고 |
|---|---|---|---|
| 1 | ✅ 완료 | 없음 | `feat/external-match-pin`. 결정 로그 D175~D178 |
| 2 | ⬜ 대기 | 없음 | PR1 선행(완료) |
| 3 | ⬜ 대기 | 없음 | PR2 선행 |
