...
| 9 | 제외 키워드와 소스 헬스 체크 | F+J | `feat/external-exclude-health` | `0011_external_source_probe` | D196~D197 |
...
- **PR9(제외 키워드·헬스)를 뒤로 뺀 이유**: 어떤 키워드를 제외해야 하는지는 실제 소스를
  붙여 봐야 안다. 헬스 체크도 소스가 여럿이어야 의미가 있다. 순서를 앞당기고 싶은
  유혹이 있었지만(PR5 의 네이버 쇼핑은 제외어가 특히 필요하다), 추측으로 만든 제외어
  목록보다 실측 후 만든 목록이 낫다고 판단했다.
...
## PR9 — 제외 키워드와 소스 헬스 체크 (방안 F+J)

> 브랜치 `feat/external-exclude-health` · 마이그레이션 `0011_external_source_probe` · 결정 D196~D197
...
초안(PR5~7 실측으로 확정): `잔`, `글라스`, `디캔터`, `미니어처`, `공병`, `굿즈`, `안주`,
`선물세트`, `쇼핑백`, `보관함`.
...
3. probe 행이 20개를 넘으면 오래된 것부터 삭제된다
4. 연속 실패 3회 이상이면 헬스가 `failing`
5. 테스트 조회는 캐시에 저장하지 않는다
6. 소스 삭제 시 probe 행 CASCADE 삭제
7. (Vitest) 배지 3상태 렌더 / 테스트 조회 버튼이 결과를 인라인 표시

### 완료 조건

- [ ] 제외 키워드 목록이 **실측 근거와 함께** 각 프리셋에 반영됨 (추측으로 채우지 않는다)
- [ ] `alembic` 왕복 성공
...
| 버전 | 포함 | 기준 |
|---|---|---|
| `v1.5.0` | PR1~PR3 | 정확도·비교 기반이 완성되는 지점. 소스가 데일리샷 하나여도 체감 개선이 크다 |
| `v1.6.0` | PR4~PR7 | 소스 확장. 실제로 붙은 사이트 수에 따라 릴리스 노트를 조정 |
| `v1.7.0` | PR8~PR9 | 보조 기능 마무리 |
...
| # | 질문 | 필요 시점 |
|---|---|---|
| Q10 | LLM 재판정의 월 비용 상한을 얼마로 둘 것인가. 텍스트만 보내므로 조회 1건당 수십 원 미만이지만, 상한 없이 켜 두는 것은 Q2 에서 우려했던 문제와 같다 | PR8 |
| Q11 | 네이버 개발자센터 애플리케이션 등록과 Untappd API 승인 신청을 누가 진행할 것인가 (계정 소유자만 가능) | PR5 |
| Q12 | PR6·PR7 사이트 중 robots·ToS 가 애매한 곳이 나오면, 보수적으로 제외할 것인가 / 개인 사용 범위로 보고 등록할 것인가 | PR6 |
| Q13 | CU·GS25·emart24 를 최종적으로 범위에서 빼는 데 동의하는가 (PR7 참조) | PR7 |
