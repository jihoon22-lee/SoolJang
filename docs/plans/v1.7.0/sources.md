...
| SEARCH-EXA / 검색·Contents | [Search](https://exa.ai/docs/reference/search), [이용약관 PDF](https://exa.ai/assets/Exa_Labs_Terms_of_Service.pdf/), [개인정보 정책](https://exa.ai/privacy-policy) | **조건부**; `x-api-key` 또는 Bearer, 사용 계약·한도 확인 | URL·제목·작성일 및 제한 text. 사용자 환경 키로 연결 확인1회·6주종 검색6회 성공(2026-09-07). 장기 본문 저장0 | Search type과 synthesis 필드를 위 결정대로 제한. 기존 `keyword` type은 현재 enum에 없으므로 예전 SDK 예제를 그대로 사용하지 않는다. 제3자 자료의 권리·완전성은 보장하지 않음; API 입력에 개인 소비 이력 포함 금지. Contents 장기 보관은 원문 권리/계약 확인 후. |
...
2. **선택 검색 제공자의 기존 계정·플랜·키 사용 승인**: 값을 채팅/기록에 요구하지 않고 앱의 암호화 설정 경로로 등록·선택한다. NAVER 신규는 API HUB, Brave는 저장권 여부, Exa는 계약/본문 권리 확인. 유료 자동 계약·재발급은 하지 않았다. Exa는 사용자가 환경 키 사용을 승인해 서버 전용 실검증에 사용했다.
...
### B05 Exa 실검증 (2026-09-07)

공식 [Search](https://exa.ai/docs/reference/search)·[Contents](https://exa.ai/docs/reference/get-contents)의 현재 계약에 따라 `fast`+`contents.text.maxCharacters=1500`, 결과10개/질의1회로 제한했다. 생성형 summary/highlights/Answers/Research/outputSchema 요청은 0이다. 앱 전달 발췌는 500자로 줄이며 원문/이미지를 DB나 실행 증거에 저장하지 않는다.

위스키·와인·전통주·사케·백주·맥주 각각 1질의, 총6회에서 결과60개/발췌60개를 취득했다. 주종별 고유 도메인은 10/10/10/8/8/9개다. 이는 실제 제공자 text 취득률이며 정확한 판본 정답률·원문 직접 검증률·데일리샷 미등록 검증이 아니다. 일본어/중국어 리뷰 종류는 명시적인 분류 근거가 부족하면 unknown으로 유지한다. 네이버·Brave는 요청/응답 contract만 자동 검증했고 실제 키 연결은 미실행이다.
