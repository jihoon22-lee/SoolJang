...
### 7.4 신뢰 구간과 매칭 고정(Task 34 PR1)

**문제**: `adapter`가 찾은 후보 중 점수가 가장 높은 하나를 조용히 채택하면, 이름이
비슷한 다른 술을 정답처럼 보여주는 일이 생긴다(실측 — 접두사만 같은 다른 증류소, 수식어가
붙어 검색어와 완전히 겹치지 않는 정식 상품명 등). 사용자가 확인·정정할 수단이 없으면
캐시가 `ttl_hours` 동안 오답을 그대로 붙잡는다.

**신뢰 구간 3분할**: 검색 후보의 최고 점수를 두 임계값으로 나눈다.

| 구간 | 조건 | 동작 |
|---|---|---|
| 자동 채택 | `score ≥ _AUTO_ACCEPT`(0.85) | 지금처럼 바로 값을 보여준다. `needs_confirmation=false` |
| 확인 필요 | `_MIN_CANDIDATE ≤ score < _AUTO_ACCEPT`(0.5~0.85) | 값은 보여주되 `needs_confirmation=true`. 후보 목록(최대 5개, 점수 내림차순)을 함께 노출 |
| 후보 없음 | `score < _MIN_CANDIDATE`(0.5) | 값 없이 후보만 노출 — 사용자가 직접 고를 수 있게 |

후보 목록은 접두사 게이트(§7.2 코드 주석) 통과 여부와 무관하게 **검색으로 찾은 전체
후보** 중에서 고른다 — 게이트가 정답을 잘못 걸러내는 경우에도 사용자가 목록에서 직접
고를 수 있어야 한다.

**매칭 고정**: 사용자가 후보(또는 조회 결과) 중 하나를 "이걸로 고정"하면
`external_product_match`(소스·제품 쌍마다 1개, `external_url`·`external_name`·
`external_key`·`confirmed_at`)에 저장한다. 절대 규칙 7 에 따라 `external_url` 은
`NOT NULL` 이다. 이후 조회는 이 고정을 최우선으로 쓴다 — 검색·점수 재계산을 건너뛴다.

고정된 소스를 조회하는 경로는 소스 형태에 따라 둘로 갈린다:

| 소스 형태 | 동작 | 이유 |
|---|---|---|
| 상세 페이지가 있는 소스(HTML 모드, 또는 `result_fields` 없는 JSON 모드) | 검색 자체를 건너뛰고 `external_url` 을 바로 조회 | 왕복 1회 절감 + 검색 결과 순위 변동에 영향받지 않음 |
| `search.result_fields` 를 쓰는 JSON 모드(데일리샷 등) | 검색은 하되, 후보 선택을 유사도 대신 **`external_url` 일치**로 한다 | 이 모드는 상세 페이지를 아예 조회하지 않는다 — 값이 검색 응답 안에만 있어서, 검색을 건너뛰면 가져올 값이 없다 |

두 번째 경로에서 고정된 URL 이 검색 결과에 더 이상 없으면(상품 단종·개편)
`degraded=true` + 후보 목록을 반환한다 — 조용히 유사도 매칭으로 되돌아가지 않는다.
그러면 고정의 의미가 사라진다.

**SSRF 재검증**: 고정 URL 은 저장 시점(`POST /products/{id}/external-matches`)과 조회
시점 양쪽에서 같은 `same_host` 검사로 검증한다. 저장 후에 소스의 `base_url` 이 바뀔 수
있고, 클라이언트가 보낸 URL 을 그대로 믿을 이유도 없다.

**자동 고정은 하지 않는다.** 고정은 항상 사용자의 명시적 조작으로만 생긴다 — 그렇지
않으면 오답을 영구화할 위험이 정확도 개선이라는 목적과 정면으로 배치된다.
