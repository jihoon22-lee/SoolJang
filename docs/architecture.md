...
### 7.2.1 표준 필드 스키마 (Task 34 PR3)

`adapter_spec`이 뽑은 값은 아래 표준 키로 정규화된다. **표준 키가 아닌 값은 버리지 않고
`extra`에 그대로 담는다** — 기존 소스(데일리샷의 `가격`·`평점`·`리뷰수` 같은 한글 키)가
깨지지 않는 것이 중요하다. 비교 표에 안 잡힐 뿐 값은 그대로 보인다.

| 키 | 타입 | 비고 |
|---|---|---|
| `price_krw` | int | 실제 판매가. `"89,000원"` 같은 문자열도 숫자로 바꾼다 |
...
| `in_stock` | bool \| null | `"품절"`·`"재고있음"` 같은 표기도 해석한다 |

**파생값은 저장하지 않는다**(§8 절대 규칙 6). `rating_normalized`(0~5 환산)와 100ml당
가격은 API 응답을 조립할 때 계산한다. 이 앱은 이미 `price_per_100ml` 파생 지표를 쓰고
있어, 내 실평단가와 같은 단위로 나란히 놓고 볼 수 있다 — 그게 이 기능의 실질 가치다.

**캐시 호환** — 스냅샷에 `version`을 넣고 `SNAPSHOT_VERSION`보다 낮은 행은 TTL과 무관하게
stale로 취급한다. 캐시는 정의상 언제든 버려도 되는 데이터라 마이그레이션 스크립트를 쓸
이유가 없다 — 다음 조회에서 자연스럽게 새 모양으로 교체된다.
